...
def setup_data(app):
    ...
    with app.app_context():
        u = User(username="ric", full_name="Riccardo", operator_code="RIC",
                 role="user", is_admin=False, is_active=True, locale="it")
        u.set_password("x")
        db.session.add(u); db.session.flush()

        sm = Substance(name="SM", smiles="CCO",
                       molecular_formula="C2H6O", molecular_weight=46.07)
        cat = Substance(name="Cat", smiles="O",
                        molecular_formula="H2O", molecular_weight=18.02)
        prod = Substance(name="Prod", smiles="CC=O",
                         molecular_formula="C2H4O", molecular_weight=44.05)
        db.session.add_all([sm, cat, prod]); db.session.flush()

        sm_lot = InventoryItem(substance_id=sm.id, batch_code="SM-A",
                               quantity_g=100.0, initial_quantity_g=100.0,
                               is_active=True)
        cat_lot = InventoryItem(substance_id=cat.id, batch_code="CAT-A",
                                quantity_g=100.0, initial_quantity_g=100.0,
                                is_active=True)
        db.session.add_all([sm_lot, cat_lot]); db.session.flush()

        rxn = Reaction(code="RX-1", template_code="OXID",
                       status="published", title="Test oxidation")
        ...
        db.session.add_all([
            ...
            ReactionComponent(reaction_id=rxn.id, substance_id=cat.id,
                              role="catalyst", position=1, equivalents=1.0),
        ...
        return {
            ...
            "sm_id": sm.id, "cat_id": cat.id, "prod_id": prod.id,
        }
...
def test_run_setup_page_shows_components(app, client, setup_data):
    _login(client, "ric")
    csrf = _csrf(client)
    client.post(f"/runs/from/{setup_data['reaction_id']}",
                data={"csrf_token": csrf})
    with app.app_context():
...
def test_full_lifecycle_via_routes(app, client, setup_data):
    ...
    # 2. Set scale
    r = client.post(f"/runs/{rid}/scale",
                    data={"csrf_token": csrf, "scale_mmol": "10"})
    assert r.status_code == 302

    # 3. Bind SM lot + actual
    client.post(f"/runs/{rid}/component/{comp_ids['starting_material']}/lot",
                data={"csrf_token": csrf, "lot_id": setup_data["sm_lot_id"]})
    client.post(f"/runs/{rid}/component/{comp_ids['starting_material']}/actual",
                data={"csrf_token": csrf, "actual": "0.46"})  # ~10 mmol × 46.07/1000

    # 4. Bind catalyst lot + actual
    client.post(f"/runs/{rid}/component/{comp_ids['catalyst']}/lot",
                data={"csrf_token": csrf, "lot_id": setup_data["cat_lot_id"]})
    client.post(f"/runs/{rid}/component/{comp_ids['catalyst']}/actual",
                data={"csrf_token": csrf, "actual": "0.18"})

    # 5. Start the run
    r = client.post(f"/runs/{rid}/start", data={"csrf_token": csrf},
                    follow_redirects=False)
    assert r.status_code == 302
    with app.app_context():
    ...
    client.post(f"/runs/{rid}/component/{prod_cid}/actual",
                data={"csrf_token": csrf, "actual": "0.3", "unit": "g"})

    # 7. Complete the run
    r = client.post(f"/runs/{rid}/complete",
                    data={"csrf_token": csrf})
    assert r.status_code == 302
    with app.app_context():
        run = db.session.get(Run, rid)
        ...
        lots = db.session.query(Inv).filter(
            Inv.source_run_id == rid).all()
        assert len(lots) == 1
...
def test_cannot_create_run_from_draft_template(app, client, setup_data):
    ...
    with app.app_context():
        ...
        rxn.status = "draft"
        db.session.commit()
    r = client.post(f"/runs/from/{setup_data['reaction_id']}",
...
def test_scale_input_value_unit_remembered(app, client, setup_data):
    ...
    client.post(f"/runs/{rid}/scale",
                data={"csrf_token": csrf, "scale_amount": "500",
                      "scale_unit": "mg"})
...
def test_toggle_checklist_returns_partial_for_htmx(app, client, setup_data):
    ...
    with app.app_context():
        ...
        item = RunChecklistItem(run_id=rid, text="Test item", position=0,
                                is_done=False)
        db.session.add(item); db.session.commit()
        item_id = item.id

    # Toggle via HTMX header
    r = client.post(f"/runs/{rid}/checklist/{item_id}/toggle",
                    data={"csrf_token": csrf},
...
def test_set_actual_returns_204_for_htmx(app, client, setup_data):
    ...
    with app.app_context():
        run = db.session.query(Run).first()
        rid = run.id
        cid = next(c.id for c in run.components
