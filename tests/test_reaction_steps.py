...
def test_checklist_constraint_xor(app):
    ...
    with app.app_context():
        ...
        db.session.add(rxn)
        db.session.flush()
        step = ReactionStep(reaction_id=rxn.id, position=0, kind="workup", title="Workup")
        db.session.add(step)
        db.session.flush()

        item = ChecklistItem(reaction_id=rxn.id, step_id=step.id, position=0, text="Bad")
        db.session.add(item)
...
def test_reaction_step_with_checklist(app):
    with app.app_context():
        ...
        db.session.add(rxn)
        db.session.flush()
        step = ReactionStep(reaction_id=rxn.id, position=0, kind="extraction", title="Estrazione")
        db.session.add(step)
        db.session.flush()
        db.session.add_all(
            [
...
def test_add_step_route(client, app):
    ...
    resp = client.post(
        ...
        data={"title": "Workup acquoso", "kind": "workup", "description": "Spegnere con NH4Cl"},
        follow_redirects=False,
    )
    assert resp.status_code == 302
    with app.app_context():
...
def test_add_step_component(client, app):
    with app.app_context():
        ...
        db.session.flush()
        step = ReactionStep(reaction_id=rxn.id, position=0, kind="workup", title="WUP")
        db.session.add(step)
        db.session.commit()
        sid = step.id
        sub_id = sub.id
