...
from stoic_eln.services.hazard_phrases import (
    parse_codes as _parse_phrase_codes,
...
def list_view():
    ...
    query = query.order_by(func.lower(Substance.name).asc())
...
@bp.route("/<int:substance_id>/low_stock", methods=["POST"])
@login_required
@supervisor_required
def update_low_stock(substance_id: int):
    """Update low-stock thresholds for a substance."""
    sub = db.session.get(Substance, substance_id)
    if sub is None:
        abort(404)
    ...
    sub.low_stock_threshold_g = _parse("low_stock_threshold_g")
    sub.low_stock_threshold_mL = _parse("low_stock_threshold_mL")
    db.session.commit()
    log_event(
        action="update_low_stock",
        entity_type="substance",
        entity_id=sub.id,
        details={
            "g": sub.low_stock_threshold_g,
            "mL": sub.low_stock_threshold_mL,
        },
    )
    flash(_("Soglie aggiornate."), "success")
    return redirect(url_for("substances.detail", substance_id=sub.id))
