...
def _header_footer(canvas_obj: canvas.Canvas, doc, *, filters,
    ...
    if filters.date_from or filters.date_to:
        bits.append(
            f"Date: {filters.date_from or '—'} → {filters.date_to or '—'}"
        )
    if filters.user_id:
...
def render_audit_log_pdf(
    *,
    ...
    cell_style = ParagraphStyle("cell", parent=base, fontSize=7,
                                  fontName="Times-Roman", leading=9)
    head_style = ParagraphStyle("head", parent=base, fontSize=8,
                                  fontName="Times-Bold", leading=10)
    ...
    for e in events:
        ...
        rows.append([
            Paragraph(_esc(e.created_at.strftime("%Y-%m-%d %H:%M:%S")
                           if e.created_at else ""), cell_style),
            Paragraph(_esc(user_name), cell_style),
            Paragraph(_esc(label), cell_style),
            Paragraph(_esc(e.entity_type or ""), cell_style),
            Paragraph(_esc(str(e.entity_id) if e.entity_id is not None else ""),
                      cell_style),
    ...
    table.setStyle(TableStyle([
        ...
        ("ROWBACKGROUNDS", (0, 1), (-1, -1),
         [colors.white, colors.HexColor("#f9f9f6")]),
    ]))
